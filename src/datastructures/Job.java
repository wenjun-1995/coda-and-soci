...
public class Job implements Comparable<Job> {
  ...
  public boolean jobExist;//ruan
  public int preIdentjobID;
  public int identjobID;
  public boolean jobDead;
  public boolean jobAddedMark;
  public boolean extJobMark;
  public int origJobID;
  /**
   * Constructor for Job.
   * 
   * @param jobName
  ...
  public Job(String jobName, int jobID) {
    ...
    //ruan
    //StartTimeOfJobs = new HashMap<Integer, Double>();
    identjobID=jobID;
    //identjobID=0;
    origJobID=0;
  ...
  //ruan
  public void setIdentJobID(int identjobID) {
	  this.identjobID = identjobID;
  }
  ...
  public void arrangeTasks(int numRacks, int machinesPerRack, double deadlineMult) { //ruan: deadlineMult>1
    ...
    for (Task t : tasks) {
      if (t.taskType == TaskType.MAPPER) {
        ...
        int fromRack = convertMachineToRack(mt.getPlacement(), machinesPerRack);//ruan: machinesPerRack =1
      ...
      if (t.taskType == TaskType.REDUCER) {
        ...
        addAscending(tasksInRacks[toRack], rt); //ruan: add reducetask into tasksInRacks[toRack] according to shuffleBytesLeft
    ...
    calcAlphaDeadline(numRacks, deadlineMult); //ruan: deadlineMult>1
  }
  ...
  private void coalesceMappers(int numRacks) {
    ...
    tasks.removeAll(newMappers); //ruan: remove all tasks contained in newMappers from tasks
    newMappers.clear();
  ...
  private void addAscending(Vector<ReduceTask> coll, ReduceTask rt) {
    ...
    for (; index < coll.size(); index++) {
      ...
      //  break;
      //}
    	if (coll.elementAt(index).actualStartTime > rt.actualStartTime)
    		break;
    }
    coll.add(index, rt);
  }
  ...
  public void decreaseShuffleBytesPerRack(int rackID, double decreaseBy) {
	//System.out.print("shuffle rackid "+rackID+" shufflebytes "+shuffleBytesPerRack[rackID]);
    shuffleBytesPerRack[rackID] -= decreaseBy;
    //System.out.println("after shufflebytes "+shuffleBytesPerRack[rackID]+" "+decreaseBy);
  ...
  private void resetJobStates() {
    ...
    jobExist=false;//ruan: job has been added into jobs according to its name
    jobDead=false;
    jobAddedMark=false;
    extJobMark = false;
  }
  ...
  private void calcAlphaDeadline(int numRacks, double deadlineMult) {
    ...
    alpha = Math.max(Utils.max(sendBytes), Utils.max(recvBytes)); // ruan: the total size of a coflow

    // Calculate deadline in millis
    deadlineDuration = alpha / Constants.RACK_BYTES_PER_SEC * deadlineMult * 1000; //ruan: give out the deadline of a coflow
  }
