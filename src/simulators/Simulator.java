...
public abstract class Simulator {
  ...
  //ruan
  protected Vector<Flow>[] flowsInJobInRacks;
  ...
  public Simulator(
    ...
    /*
    System.out.println(traceProducer.getClass().getName());
    try {
		System.in.read();
	} catch (IOException e) {
  ...
  protected void initialize(TraceProducer traceProducer) {
    ...
    //ruan
    for (Job j : jobs) {
    	System.out.println(j.jobID+" "+j.actualStartTime+" "+j.identjobID);
    }
    System.out.println("Test sort2 in Simulator.java. Please input any key.");
    try {
		System.in.read();
	} catch (IOException e) {
		// TODO Auto-generated catch block
  ...
  private void mergeTasksByRack() {
    ...
    for (Job j : jobs) {
      if (!ignoreThisJob(j)) {
        double deadlineMult = 1.0 + deadlineGen.nextDouble() * deadlineMultRandomFactor;//ruan: deadlineMultRandomFactor=1
  ...
  public void simulate(int EPOCH_IN_MILLIS) {
    ...
    for (CURRENT_TIME = 0; CURRENT_TIME < Constants.SIMULATION_ENDTIME_MILLIS
      ...
      //ruan:
      //System.out.println("Curtime:"+CURRENT_TIME+" Endtime: "+Constants.SIMULATION_ENDTIME_MILLIS);
      int jobsAdded = 0;
      ...
      for (; curJob < TOTAL_JOBS; curJob++) {
        ...
        if (j.actualStartTime > CURRENT_TIME + EPOCH_IN_MILLIS) {
        	//System.out.println("Do not admit "+j.jobName);
          break;
        }
        ...
        //System.out.println(j.jobName+" "+j.actualStartTime+" "+CURRENT_TIME+" "+EPOCH_IN_MILLIS);
        // One job added
        j.wasAdmitted = true;
        jobsAdded++;
        uponJobAdmission(j); //ruan:set the start time of job and task
      }
      ...
      for (long i = 0; i < EPOCH_IN_MILLIS; i += Constants.SIMULATION_QUANTA) {
        ...
        //System.out.println("epoch: "+EPOCH_IN_MILLIS+" quanta:"+Constants.SIMULATION_QUANTA); 
        long curTime = CURRENT_TIME + i;
        onSchedule(curTime);

        //System.out.println("curtime: "+curTime+" "+curTime % Constants.SIMULATION_SECOND_MILLIS);
        // Print progress
  ...
  public double printStats(boolean doPrint) {
    ...
    if (doPrint) {
      ...
      System.out.println("admit jobs num: " + admitCount + " ignor num: " + ignoreCount+ " cct: " + sumDur/admitCount);
      if (considerDeadline) {
